...
char csvChar[BUFFER_SIZE];
PString csvString(csvChar, sizeof(csvChar));
int numStation = 0;
unsigned int treasuresNum = 0;
unsigned long long startTime = 0;
unsigned long long finishTime = 0;
float minPulseRate = 100000.0f;
float maxPulseRate = -1.0f;
float sumPulseRate = 0.0f;
int numPulseRate = 0;
...
void setup() 
{
  ...
  */  Serial.begin(115200);
  ...
  // antet echipa
  csvString = "Team ID,Bucharest1HnS@Bucharest1S" + endOfLine;
  // antet coloane
  csvString = ",Time Stamp (dd.mm.yy - hh.mm.ss),Recorded values" + endOfLine;
}
...
void writeCsvToFile() {
  ...
  // reset csvString
  csvString.begin(); // s-ar putea sa crape: buffer overflow

}

void addTreasures() {
  if (0 < numStation && numStation < 6) {

  }
}

void updateCSVString() {
  if (numStation == 0) {
    // TODO(ioana): update starttime
    //startTime = 
    csvString.println("Start Information,%lld", startTime);
  } else if (numStation == 6) {
    // TODO(ioana): update finishtime
    //finishTime = 
    csvString.println("Finish Information,%lld", finishTime);
  } else {
    csvString.println("Station %d,%lld", i, timestamp);
  }

  // BioData:
  csvString.println("Bio-Data,%s", formatTime(timestamp));
  
  int i = 0;
  csvString.println("Body Temperature [*C],,%f", toString(bioData[i]); //0
  ++i;

  csvString.println("Pulse Rate [bpm],,%f" + bioData[i]); //1
  minPulseRate = (minPulseRate > bioData[i]) ? bioData[i] : minPulseRate;
  maxPulseRate = (maxPulseRate < bioData[i]) ? bioData[i] : maxPulseRate;
  sumPulseRate += bioData[i];
  ++numPulseRate;
  ++i;

  csvString.println("Number of steps [decimal],,%f" + bioData[i]); //2
  ++i;

  csvString.println("Distance traveled [m],," + bioData[i]); //3
  ++i;

  // Environmental data:
  csvString.println("Environmental data,%s", formatTime(timestamp));
  csvString.println("Air Temperature [*C],,%f" + bioData[i]);
  ++i;  
  csvString.println("Humidity [%],,%f" + bioData[i]));
  ++i;
  ...
  ++numStation;
  if (0 < numStation && numStation < 6) {
    // TODO(ioana): add treasures
    writeTreasures()
  }
}

void createFooter() {
  csv.println("Total identified treasures,%u", treasuresNum);
  csv.println("Total time (start - finish),%lf sec", ((double)(finishTime - startTime))/1000);
  csv.println("Min. Pulse Rate,%f bpm", minPulseRate);
  csv.println("Max. Pulse Rate,%f bpm", maxPulseRate);
  csv.println("Average Pulse Rate,%f bpm", (sumPulseRate/numPulseRate));
  writeCsvToFile();
}
...
int scanTweetTag()
{
  ...
  // TODO: ioana : fara treasures (functie separata)
  updateCSVString();
